...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'mdl-card mdl-shadow--3dp mdl-cell ' . $post_class ); ?>>
	...
	<header class="mdl-card__title">
		<h2 class="mdl-card__title-text"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'dhhc' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
	</header><!-- /.entry-header -->
	...
	<div class="mdl-card__supporting-text">
		<?php if ( 'post' === get_post_type() ) : ?>
			<p class="entry-meta">
				<?php
					dhhc_article_posted_on();
					
					$num_comments = get_comments_number();
					
					if ( comments_open() ) :
							if ( 0 === $num_comments ) {
								//$comments = __( 'No Comments', 'dhhc' );
							} elseif ( $num_comments > 1 ) {
								$comments = $num_comments . ' ' . __( 'Comments', 'dhhc' );
							} else {
								$comments = '1 ' . __( 'Comment', 'dhhc' );
							}
	...
	<footer class="mdl-card__actions mdl-card--border">
		<a href="<?php echo get_the_permalink(); ?>" class="mdl-button mdl-js-button"><?php _e( 'more', 'dhhc' ); ?></a>
	</footer><!-- .entry-meta -->
